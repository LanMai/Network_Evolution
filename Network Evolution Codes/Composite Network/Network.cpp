...
void Network::BurstTimeOnsetSporadic()
{
		...
		for (int j=0; j<NoP; j++)
		{
			if ( NeuronPopBurstData[j][i]>0 )
			{
				...
				if ( BurstData[initialT][j]>0 )
				{
					if ( TonicNeuron[j]==1 ) 
					{
						if (initialT-7>0) NeuronActBegin[j][i] = initialT - 7; // changed from 10 to 7
	...
	for (int i=0; i<PopBurstCount; i++)
	{
		for (int j=0; j<NoP; j++)
		{
			if ( NeuronPopBurstData[j][i]>0 )
			{
				...
				if ( TonicNeuron[j]==1 ) 
				{
					if (PopActBeginEnd[i][1]+7<BLcount-1) NeuronActEnd[j][i] = PopActBeginEnd[i][1] + 7; // changed from 10 to 7
					else NeuronActEnd[j][i] = BLcount;
				}
				else
				{
	...
	SporadicFrac100 = 100*double(SporadicBurstCount)/double(SporadicBurstPossible);

	cout << "process_id: " << process_id  << "\t SporadicBurstPossible = " << SporadicBurstPossible 
		<< "\t SporadicBurstCount = " << SporadicBurstCount 
		<< "\t SporadicFrac = " << SporadicFrac100 << endl;	
...
netconfig_result Network::EvalPopNeuronBurstCC()
{
	...
	else // (NoPoPburst > 3)
	{
		...
		//nr.eval2 = double(BLcount*NoNe)-SumExcitatorySporadic();
		//nr.eval2 = pow(PopBurstInterval*160/1000-SetPopBurstFreq,2);
		nr.eval2 = double( NoN*NoN -(EEsum+IIsum+EIsum+IEsum) );
		for (int i=0; i<=CCxbins; i++)
		{
			nr.CCx[i] = CCx[i];
			nr.CCy[i] = CCy[i];
...
{
	ostringstream stream;
	stream << "NeuronProp160A" << job_id << ".txt";
...
netconfig_cond Network::MutatateNetworkHybrid(int iter, int PsuedoMutate)
{
	srand ( time(NULL)+process_id+iter*1010);
	double RN = double(rand()) / double(RAND_MAX);
	cout << "process_id: " << process_id << " RN = " << RN << endl;
	...
	if (PsuedoMutate==0)
	{
		...
		double w;
		double pMutation = 0.35;
		if ( RN < pMutation )
		{
			//RN = 2*RN;
			w = pMutation*0.125;
			//cout << "process_id: " << process_id << " RN = " << RN << " w = " << w << endl;
			if ( RN < w ) MutationPartialColTranslocateENG1(iter);
			if ( RN >= w && RN < 2*w ) MutationPartialColTranslocateEI1(iter);
			if ( RN >= 2*w && RN < 3*w ) MutationPartialColTranslocateIE1(iter);
			if ( RN >= 3*w && RN < 4*w ) MutationPartialColTranslocateING1(iter);
			if ( RN >= 4*w && RN < 5*w ) MutationPartialRowTranslocateENG1(iter);
			if ( RN >= 5*w && RN < 6*w ) MutationPartialRowTranslocateEI1(iter);
			if ( RN >= 6*w && RN < 7*w ) MutationPartialRowTranslocateIE1(iter);
			if ( RN >= 7*w ) MutationPartialRowTranslocateING1(iter);
		}
		else
		{			
			RN = (RN-pMutation);
			w = (1-pMutation)*0.5*0.125;
			//cout << "process_id: " << process_id << " RN = " << RN << " w = " << w << endl;
			if ( RN < w ) MutationSynColAddENG1(iter);
			if ( RN >= 1*w && RN < 2*w ) MutationSynRowAddENG1(iter);
			if ( RN >= 2*w && RN < 3*w ) ColAddEI(iter);
			if ( RN >= 3*w && RN < 4*w ) RowAddEI(iter);
			if ( RN >= 4*w && RN < 5*w ) ColAddIE(iter);
			if ( RN >= 5*w && RN < 6*w ) RowAddIE(iter);
			if ( RN >= 6*w && RN < 7*w ) ColAddII(iter);
			if ( RN >= 7*w && RN < 8*w) RowAddII(iter);

			if ( RN >= 8*w && RN < 9*w ) MutationSynColDeleteENG1(iter);
			if ( RN >= 9*w && RN < 10*w ) MutationSynRowDeleteENG1(iter);
			if ( RN >= 10*w && RN < 11*w ) ColDeleteEI(iter);
			if ( RN >= 11*w && RN < 12*w ) RowDeleteEI(iter);
			if ( RN >= 12*w && RN < 13*w ) ColDeleteIE(iter);
			if ( RN >= 13*w && RN < 14*w ) RowDeleteIE(iter);
			if ( RN >= 14*w && RN < 15*w ) ColDeleteII(iter);
			if ( RN >= 15*w ) RowDeleteII(iter);
			...
			if ( RN >= 5*w && RN < 6*w ) RowAddIE(iter);
			if ( RN >= 6*w && RN < 7*w ) ColAddII(iter);
			if ( RN >= 7*w ) RowAddII(iter);*/

			/*RN = (RN-pMutation);
			w = (1-pMutation)*0.125;
			if ( RN < w ) MutationSynColDeleteENG1(iter);
			if ( RN >= 1*w && RN < 2*w ) MutationSynRowDeleteENG1(iter);
			if ( RN >= 2*w && RN < 3*w ) ColDeleteEI(iter);
			if ( RN >= 3*w && RN < 4*w ) RowDeleteEI(iter);	
			if ( RN >= 4*w && RN < 5*w ) ColDeleteIE(iter);
			if ( RN >= 5*w && RN < 6*w ) RowDeleteIE(iter);
			if ( RN >= 6*w && RN < 7*w ) ColDeleteII(iter);
			if ( RN >= 7*w ) RowDeleteII(iter);*/
...
void Network::ColDeleteII(int iter)
{
	...
	for (int k=0; k<1; k++)
	{
		while ( synCount==0 )
		{
			...
			for (int i=0; i<NoNe; i++) 
			{
				synCount = synCount + conMatII[i][neuron[0]];
			}
		}
	}
...
void Network::RowDeleteII(int iter)
{
	...
	for (int k=0; k<1; k++)
	{
		while ( synCount==0 )
		{
			...
			for (int i=0; i<NoNe; i++) 
			{
				synCount = synCount + conMatII[neuron[0]][i];
			}
		}
	}
...
void Network::ColDeleteEI(int iter)
{
	...
	for (int k=0; k<1; k++)
	{
		while ( synCount==0 )
		{
			...
			for (int i=0; i<NoNe; i++) 
			{
				synCount = synCount + conMatEI[i][neuron[0]];
			}
		}
	}
...
void Network::RowDeleteEI(int iter)
{
	...
	for (int k=0; k<1; k++)
	{
		while ( synCount==0 )
		{
			...
			for (int i=0; i<NoNe; i++) 
			{
				synCount = synCount + conMatEI[neuron[0]][i];
			}
		}
	}
...
void Network::ColDeleteIE(int iter)
{
	...
	for (int k=0; k<1; k++)
	{
		while ( synCount==0 )
		{
			...
			for (int i=0; i<NoNe; i++) 
			{
				synCount = synCount + conMatIE[i][neuron[0]];
			}
		}
	}
...
void Network::RowDeleteIE(int iter)
{
	...
	for (int k=0; k<1; k++)
	{
		while ( synCount==0 )
		{
			...
			for (int i=0; i<NoNe; i++) 
			{
				synCount = synCount + conMatIE[neuron[0]][i];
			}
		}
	}
