...
void Network::MutationNeuronSwitchENG(int iter)
{
	...
	int MaxNeuronForSwitch = 5;
	int* neuron;
	neuron = new int[MaxNeuronForSwitch];
	int RN1_copy = 0;
	int RN2_copy = 0;

	double RN1;
	for (int k=0; k<MaxNeuronForSwitch; k++)
	{
		do
		{
			RN1 = double(rand()) / double(RAND_MAX);
	...
	RN1_copy = RN2_copy;
	int TotalNeuronForSwitch = 2 + floor(double(MaxNeuronForSwitch-2)*RN1);	
	int TotalSwitchingLoop = TotalNeuronForSwitch - 1;
	cout << "process_id: " << process_id << "\t TotalNeuronForSwitch " << TotalNeuronForSwitch << endl;

	int SynTemp;
	for (int k=0; k<TotalSwitchingLoop; k++)
	{
		// Row swaping (begin) within Excitatory NG
		for (int i=0; i<NoNe; i++)
		{
			SynTemp = conMatEE[neuron[k]][i];
			conMatEE[neuron[k]][i] = conMatEE[neuron[k+1]][i];
			conMatEE[neuron[k+1]][i] = SynTemp;
		}

		// Column swaping (begin) within Excitatory NG
		for (int i=0; i<NoNe; i++)
		{
			SynTemp = conMatEE[i][neuron[k]];
			conMatEE[i][neuron[k]] = conMatEE[i][neuron[k+1]];
			conMatEE[i][neuron[k+1]] = SynTemp;
		}
		cout << "process_id: " << process_id << "\t TotalSwitch: " << TotalSwitchingLoop << "\t Switch: " << k << "\t neuron " << neuron[k] << " in ENG is swaped by neuron " << neuron[k+1] << endl;
	}
